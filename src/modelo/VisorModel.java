...
public class VisorModel {
    ...
    /**
     * Reemplaza la lista maestra y el mapa de rutas para el contexto de trabajo
     * actual,
     ...
     * 
     * @param nuevoModelo    El nuevo DefaultListModel que actuará como lista
     *                       maestra.
     * @param nuevoMapaRutas El mapa de rutas correspondiente al nuevo modelo.
     * @param source         El objeto controlador que inició el cambio (para
     *                       depuración).
     */
    public void setMasterListAndNotify(DefaultListModel<String> nuevoModelo, Map<String, Path> nuevoMapaRutas,
        ...
        if (currentContext != null) {
            ...
            logger.debug("[Model] Notificando a " + masterListListeners.size()
                    + " oyentes sobre el cambio de la lista maestra.");
    ...
    public void actualizarListaCompleta(DefaultListModel<String> nuevoModelo,
            Map<String, java.nio.file.Path> nuevoMapaRutas) {
    ...
    public void setCurrentZoomMode(ZoomModeEnum newMode) {
        getCurrentZoomContext().setZoomMode(newMode);
    }

    public int getImageOffsetX() {
        return getCurrentZoomContext().getImageOffsetX();
    }

    public void setImageOffsetX(int x) {
        getCurrentZoomContext().setImageOffsetX(x);
    }

    public void addImageOffsetX(int deltaX) {
        setImageOffsetX(getImageOffsetX() + deltaX);
    }

    public int getImageOffsetY() {
        return getCurrentZoomContext().getImageOffsetY();
    }

    public void setImageOffsetY(int y) {
        getCurrentZoomContext().setImageOffsetY(y);
    }

    public void addImageOffsetY(int deltaY) {
        setImageOffsetY(getImageOffsetY() + deltaY);
    }
    ...
    public boolean isEnModoProyecto() {
        return enModoProyecto;
    }
    ...
    public int getSaltoDeBloque() {
        return saltoDeBloque;
    }

    public void setSaltoDeBloque(int salto) {
        this.saltoDeBloque = salto;
    }

    public ListContext getCarouselListContext() {
        return this.carouselListContext;
    }

    public ListContext getVisualizadorListContext() {
        return this.visualizadorListContext;
    }
    ...
    public Map<String, Path> getRutaCompletaMap() {
        return getCurrentListContext().getRutaCompletaMap();
    }

    public Path getRutaCompleta(String key) {
        return getCurrentListContext().getRutaCompleta(key);
    }

    public void setZoomHabilitado(boolean b) {
        getCurrentZoomContext().setZoomHabilitado(b);
    }

    public boolean isZoomHabilitado() {
        return getCurrentZoomContext().isZoomHabilitado();
    }

    public void resetPan() {
        getCurrentZoomContext().resetPan();
    }
    ...
    public void setCarouselDelay(int delayMs) {
        ...
        if (sign == 0)
            sign = 1; // Si es 0, lo tratamos como positivo
    ...
    public void setMostrarSoloCarpetaActual(boolean mostrarSoloCarpetaActual) {
        if (isMostrarSoloCarpetaActual() != mostrarSoloCarpetaActual) {
            ...
            logger.debug("  [Model " + currentWorkMode + "] Estado mostrarSoloCarpetaActual cambiado a: "
    ...
    public void setMantenerProporcion(boolean mantenerProporcion) {
        if (isMantenerProporcion() != mantenerProporcion) {
            ...
            logger.debug(
                    "  [Model " + currentWorkMode + "] Estado mantenerProporcion cambiado a: " + mantenerProporcion);
        }
    } // --- Fin del método setMantenerProporcion ---

    public Path getCarpetaRaizActual() {
        return getCurrentListContext().getCarpetaRaizContexto();
    }
    ...
    public void setCarpetaRaizActual(Path carpetaRaiz) {
        ...
        if (currentContext != null) {
            if (!Objects.equals(currentContext.getCarpetaRaizContexto(), carpetaRaiz)) {
                logger.debug("  [VisorModel] Carpeta raíz para contexto " + currentWorkMode + " cambiada a '"
                        + carpetaRaiz + "'");
    ...
    public boolean isZoomToCursorEnabled() {
        return getCurrentZoomContext().isZoomToCursorEnabled();
    }
    ...
    public boolean isModoPantallaCompletaActivado() {
        return this.modoPantallaCompletaActivado;
    }
    ...
    public Path getCarpetaRaizDelVisualizador() {
        return (this.visualizadorListContext != null) ? this.visualizadorListContext.getCarpetaRaizContexto() : null;
    }
    ...
    public void setCarpetaRaizInicialParaVisualizador(Path carpetaRaiz) {
        if (this.visualizadorListContext != null) {
            ...
            logger.debug(
                    "  [VisorModel] Carpeta raíz INICIAL para contexto VISUALIZADOR establecida a: " + carpetaRaiz);
        }
    } // --- Fin del método setCarpetaRaizInicialParaVisualizador ---

    public boolean isCarouselShuffleEnabled() {
        return this.carouselShuffleEnabled;
    }
    ...
    public boolean isSyncVisualizadorCarrusel() {
        return this.syncVisualizadorCarrusel;
    }
    ...
    public void setSyncVisualizadorCarrusel(boolean syncActivado) {
        if (this.syncVisualizadorCarrusel != syncActivado) {
            ...
            logger.debug("[VisorModel] Sincronización Visor<->Carrusel cambiada a: "
                    + (syncActivado ? "ACTIVADO" : "DESACTIVADO"));
        }
    } // --- Fin del método setSyncVisualizadorCarrusel ---

    public double getZoomCustomPercentage() {
        return this.zoomCustomPercentage;
    }
    ...
    public Path getUltimaCarpetaCarrusel() {
        return this.ultimaCarpetaCarrusel;
    }

    public void setUltimaCarpetaCarrusel(Path path) {
        this.ultimaCarpetaCarrusel = path;
    }

    public String getUltimaImagenKeyCarrusel() {
        return this.ultimaImagenKeyCarrusel;
    }

    public void setUltimaImagenKeyCarrusel(String key) {
        this.ultimaImagenKeyCarrusel = key;
    }

    public boolean isLiveFilterActive() {
        return this.liveFilterActive;
    }

    public void setLiveFilterActive(boolean liveFilterActive) {
        this.liveFilterActive = liveFilterActive;
    }

    public boolean isGridMuestraEstado() {
        return this.gridMuestraEstado;
    }

    public void setGridMuestraEstado(boolean gridMuestraEstado) {
        this.gridMuestraEstado = gridMuestraEstado;
    }

    public boolean isProjectExportPanelVisible() {
        return this.isProjectExportPanelVisible;
    }

    public void setProjectExportPanelVisible(boolean isVisible) {
        this.isProjectExportPanelVisible = isVisible;
    }

    public Path getRutaProyectoActivoConNombre() {
        return rutaProyectoActivoConNombre;
    }

    public void setRutaProyectoActivoConNombre(Path rutaProyectoActivoConNombre) {
        this.rutaProyectoActivoConNombre = rutaProyectoActivoConNombre;
    }
