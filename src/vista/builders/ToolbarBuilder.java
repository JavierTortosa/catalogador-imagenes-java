...
public class ToolbarBuilder {
    ...
    public JToolBar buildSingleToolbar(ToolbarDefinition toolbarDef) {
        ...
        if (toolbarDef.alignment() == ToolbarAlignment.FREE) {
            logger.debug("    -> Barra con alineamiento FREE detectada ('{}'). Aplicando transparencia.",
            ...
            toolbar.setBorder(null); // Sin bordes para barras FREE
        } else {
            // Para las barras normales, añadimos un recuadro con título
            TitledBorder titledBorder = BorderFactory.createTitledBorder(
                    BorderFactory.createLineBorder(new java.awt.Color(128, 128, 128, 60), 1, true),
                    toolbarDef.titulo(),
                    TitledBorder.LEFT,
                    TitledBorder.TOP,
            ...
            toolbar.setBorder(BorderFactory.createCompoundBorder(
                    titledBorder,
                    BorderFactory.createEmptyBorder(-2, 2, 1, 2))); // Ajuste para que los botones no bajen demasiado
            toolbar.setOpaque(false); // Dejamos que se vea el fondo del contenedor principal
        }
        ...
        if (toolbarDef.componentes() != null) {
            ...
            for (ToolbarComponentDefinition compDef : toolbarDef.componentes()) {
                ...
                if (compDef instanceof ToolbarButtonDefinition botonDef) {
                    ...
                    javax.swing.JLabel speedLabel = new javax.swing.JLabel(labelDef.textoDefecto(),
                    ...
                    javax.swing.JTextField textField = new javax.swing.JTextField(textFieldDef.textoPorDefecto(),
                            textFieldDef.columns());
    ...
    private JComponent crearComponenteIndividual(ToolbarButtonDefinition definition) {
        ...
        if (definition == null || definition.comandoCanonico() == null) {
            logger.error(
                    "ERROR [ToolbarBuilder.crearComponenteIndividual]: Definición de botón o comando canónico es nulo.");
            return null;
        }
        ...
        switch (definition.tipoBoton()) {
            ...
            case CHECKERED_OVERLAY_ICON_BUTTON:
                // Estos botones se crean como JButtons normales, su icono especial se pone
                // después
            ...
            default:
                logger.error("ERROR [ToolbarBuilder.crearComponenteIndividual]: Tipo de botón no reconocido: "
                        + definition.tipoBoton());
                abstractButtonComponent = new JButton();
                break;
        }
        ...
        if (abstractButtonComponent != null) {
            ...
            // 1. Obtener SIEMPRE el icono FRESCO desde IconUtils, ignorando el que pueda
            // tener la Action.
            // Esto asegura que usamos el icono del tema actual, ya que IconUtils depende de
            // ThemeManager.
            Icon finalIcon = null;
            ...
            // 2. Asignar explícitamente el icono al botón DESPUÉS de haber asignado la
            // Action.
            ...
            if (definition.tipoBoton() == ButtonType.COLOR_OVERLAY_ICON_BUTTON) {
                ...
                abstractButtonComponent.setIcon(this.iconUtils.getColoredOverlayIcon(definition.claveIcono(),
                        initialColor, targetIconWidth, targetIconHeight));
            } else if (definition.tipoBoton() == ButtonType.CHECKERED_OVERLAY_ICON_BUTTON) {
                abstractButtonComponent.setIcon(this.iconUtils.getCheckeredOverlayIcon(definition.claveIcono(),
            ...
            logger.debug("[ToolbarBuilder.crearComponenteIndividual] Construyendo DPadComponent para tipo: "
                    + definition.tipoBoton());
            final int DPAD_DISPLAY_SIZE = 32;

            if (definition.listaDeHotspots() == null || definition.listaDeHotspots().isEmpty()) {
                logger.error(
                        "ERROR [ToolbarBuilder.crearComponenteIndividual]: La definición de DPAD_CRUZ/GRID no contiene una lista de hotspots.");
            ...
            if (definition.claveIconoSecundario() != null) {
                pressedImage = (definition.scopeIconoBase() == IconScope.COMMON)
                        ? iconUtils.getRawCommonImage(definition.claveIconoSecundario())
                        : iconUtils.getRawIcon(definition.claveIconoSecundario());
            }
            ...
            logger.error(
                    "ERROR [ToolbarBuilder.crearComponenteIndividual]: No se pudo crear componente. Tipo no manejado: "
                            + definition.tipoBoton());
            return null;
        }
