...
public class DeleteAction extends AbstractAction implements ContextSensitiveAction {
    ...
    public DeleteAction(String name,
        ...
        super(name, icon);
        this.fileOpsManager = Objects.requireNonNull(fileOpsManager,
                "FileOperationsManager no puede ser null en DeleteAction");
        this.model = Objects.requireNonNull(model,
                "VisorModel no puede ser null en DeleteAction para actualizar estado enabled");
    ...
    /**
     * Actualiza el estado 'enabled' de esta Action basado en si hay una imagen
     * seleccionada en el modelo.
     * Este método debería ser llamado cuando cambia la selección de imagen.
     * El ListCoordinator o VisorController podrían ser responsables de llamar a
     * esto.
     */
    public void actualizarEstadoEnabled() {
        if (model != null) {
            setEnabled(
                    model.getSelectedImageKey() != null && model.getRutaCompleta(model.getSelectedImageKey()) != null);
        } else {
            setEnabled(false);
        }
    }
