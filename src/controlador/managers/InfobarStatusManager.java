...
public class InfobarStatusManager implements ThemeChangeListener {
        ...
        if (modoZoomBoton != null && modoZoomBoton.isEnabled()) {
            ...
            if (esVisible && model.getCurrentZoomMode() != null && iconUtils != null) {
                ...
                if (iconKey != null) {
                    ...
                    logger.warn(
                            "WARN [InfobarStatusManager]: No se encontró clave de icono para el modo: " + modoActual);
    ...
    private void actualizarLabelZoom() {
        ...
        boolean esModoManual = (modo == ZoomModeEnum.MAINTAIN_CURRENT_ZOOM
                || modo == ZoomModeEnum.USER_SPECIFIED_PERCENTAGE);
        ...
        porcentajeLabel
                .setToolTipText("Clic para establecer zoom. Fijado en: " + String.format("%.0f%%", porcentajeAMostrar)
                        + ". Zoom real: " + String.format("%.2f%%", porcentajeReal));
    ...
    private void actualizarBarraEstado() {
        ...
        if (panelBarraInferior.isVisible() != panelVisible)
            panelBarraInferior.setVisible(panelVisible);
        if (!panelVisible)
            return;
    ...
    private void actualizarRutaArchivoInferior() {
        javax.swing.JTextField rootField = registry.get("textfield.estado.carpetaRaiz");
        
        if (rootField == null)
            return;
            
        boolean esVisible = configuration.getBoolean(ConfigKeys.INFOBAR_INF_NOMBRE_RUTA_VISIBLE, true);
        if (rootField.isVisible() != esVisible) rootField.setVisible(esVisible);
        ...
        if (esVisible) {
            ...
            if (rootPath != null) {
                ...
                // Fallback: Si estamos en modo proyecto y no hay carpeta raíz, mostramos la de destino
                String destino = projectService.getCurrentProject().getExportDestinationFolder();
                if (destino != null && !destino.trim().isEmpty()) {
                    folderDisplay = "Carpeta Destino: " + destino;
                }
            }
    ...
    private void actualizarIndicadoresDeEstado() {
        ...
        boolean funcionalidadesVisorActivas = model.isVisorFunctionalityActive();
    ...
    /**
     * Configura los listeners interactivos para los controles de la barra de
     * estado,
    ...
    public void configurarListenersControles() {
        ...
        if (porcentajeLabel != null) {
            ...
            logger.warn(
                    "WARN [InfobarStatusManager]: Componente 'label.control.zoomPorcentaje' no encontrado en el registro.");
        }
        ...
        if (finalModoZoomBoton != null) {
            ...
            logger.warn(
                    "WARN [InfobarStatusManager]: No se pudo encontrar el botón con el comando canónico 'cmd.control.modoZoom'.");
        }
     ...
     * En estado habilitado, restaura la funcionalidad y actualiza al estado del
     * modelo.
     * 
    ...
    private String getIconKeyForZoomMode(ZoomModeEnum modo) {
        ...
        return uiDefService.generateModularToolbarStructure().stream()
                ...
                // 2. Aplanamos la lista de componentes de esa barra en un único Stream de
                // componentes
    ...
    public double getValorActualDelLabelZoom() {
        ...
        if (porcentajeLabel == null) {
            logger.warn(
                    "WARN [getValorActualDelLabelZoom]: El componente 'label.control.zoomPorcentaje' no se encontró en el registro.");
            return 100.0;
        }
        ...
        try {
            ...
            logger.warn("WARN [getValorActualDelLabelZoom]: No se pudo parsear el valor del label de zoom: '"
                    + textoActual + "'. Devolviendo 100.");
            return 100.0;
        }
