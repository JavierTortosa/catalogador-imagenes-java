...
public class ViewManager implements IViewManager, ThemeChangeListener, ClipboardOwner {
     ...
     *
     * @param fullScreenState true para entrar en pantalla completa, false para
     *                        salir.
     */
    public void setFullScreen(boolean fullScreenState) {
        ...
        logger.debug("  [ViewManager] Ejecutando cambio a pantalla completa. Estado solicitado: "
                + (fullScreenState ? "ACTIVADO" : "DESACTIVADO"));

        // Comprobar si el cambio es realmente necesario para evitar trabajo extra.
        // device.getFullScreenWindow() devuelve la ventana actual en pantalla completa,
    ...
    /**
     * Abre un JColorChooser para que el usuario elija un color de fondo
     * personalizado para la SESIÓN ACTUAL.
     */
    @Override
    public void requestCustomBackgroundColor() {
        ...
        if (displayPanel == null || mainFrame == null) {
            logger.error(
                    "ERROR [ViewManager]: No se puede abrir JColorChooser, falta 'frame.main' o 'panel.display.imagen' en el registro.");
            return;
        }

        java.awt.Color colorActual = displayPanel.getBackground();
        java.awt.Color colorElegido = javax.swing.JColorChooser.showDialog(mainFrame, "Seleccionar Color de Fondo",
                colorActual);
    ...
    /**
     * Punto de entrada para solicitar actualizaciones de visibilidad de componentes
     * de la UI.
     * AHORA sabe distinguir entre una solicitud para una barra de herramientas
     * completa
     * y una solicitud para un botón individual dentro de una barra.
     * 
     * @param uiElementId El identificador del componente o zona a actualizar. Para
     *                    un botón, este
     *                    será el ID de la toolbar que lo contiene (ej:
     *                    "navegacion").
     * @param configKey   La clave de configuración que cambió. Para un botón, esta
     *                    será la clave
     *                    completa de visibilidad del botón (ej:
     *                    "interfaz.boton...").
     * @param nuevoEstado El nuevo estado de visibilidad (true para visible, false
     *                    para oculto).
     */
    @Override
    public void solicitarActualizacionUI(String uiElementId, String configKey, boolean nuevoEstado) {
        logger.debug("[ViewManager] Solicitud de actualización para UI: '" + uiElementId + "' -> " + nuevoEstado
                + " (Clave: " + configKey + ")");

        // Añadimos la lógica para guardar el estado en la configuración, que es lo que
        // faltaba.
        // Esta lógica se ejecuta para TODOS los casos, ya sean botones, toolbars o
        // paneles.
        if (configKey != null && !configKey.isBlank()) {
            if (configuration != null) {
                ...
                logger.error("  ERROR: ConfigurationManager es nulo. No se puede guardar el estado para la clave: "
                        + configKey);
            }
        }
        ...
        // CASO 3: Lógica de fallback para otros componentes principales (la que ya
        // tenías)
        ...
        switch (uiElementId) {
            ...
            case "barra_de_estado":
                ...
                if (bottomStatusBar != null && bottomStatusBar.isVisible() != nuevoEstado) {
                    ...
                    necesitaRevalidateGeneral = true;
                    logger.info(
                            "  -> Visibilidad de 'Barra de Estado' (panel.estado.inferior) cambiada a " + nuevoEstado);
                }
                break;
            ...
            case "barra_de_info_imagen":
                ...
                if (topInfoPanel != null && topInfoPanel.isVisible() != nuevoEstado) {
                    ...
                    necesitaRevalidateGeneral = true;
                    logger.info("  -> Visibilidad de 'Barra_de_Info_Superior' (panel.info.superior) cambiada a "
                            + nuevoEstado);
                }
                break;
            ...
            default:
                ...
                logger.warn("  WARN [ViewManager]: uiElementId no reconocido o no manejado por ninguna lógica: '"
                        + uiElementId + "'");
                break;
        }
    ...
    /**
     * Orquesta un refresco completo de la apariencia de la UI aplicando el tema
     * actual.
     */
    @Override
    ...
    /**
     * Restablece el fondo del visor a su estado POR DEFECTO, según lo define la
     * configuración.
     */
    @Override
    ...
    public void sincronizarAccionesFormatoBarraSuperior() {
        if (this.actionMap == null)
            return;
        Action action1 = this.actionMap
                .get(AppActionCommands.CMD_INFOBAR_CONFIG_FORMATO_SUPERIOR_NOMBRE_RUTA_SOLO_NOMBRE);
        if (action1 instanceof SetInfoBarTextFormatAction) {
            ((SetInfoBarTextFormatAction) action1).sincronizarSelectedKeyConConfig();
        }
        Action action2 = this.actionMap
                .get(AppActionCommands.CMD_INFOBAR_CONFIG_FORMATO_SUPERIOR_NOMBRE_RUTA_RUTA_COMPLETA);
    ...
    public void sincronizarAccionesFormatoBarraInferior() {
        if (this.actionMap == null)
            return;
        Action action1 = this.actionMap
                .get(AppActionCommands.CMD_INFOBAR_CONFIG_FORMATO_INFERIOR_NOMBRE_RUTA_SOLO_NOMBRE);
        if (action1 instanceof SetInfoBarTextFormatAction) {
            ((SetInfoBarTextFormatAction) action1).sincronizarSelectedKeyConConfig();
        }
        Action action2 = this.actionMap
                .get(AppActionCommands.CMD_INFOBAR_CONFIG_FORMATO_INFERIOR_NOMBRE_RUTA_RUTA_COMPLETA);
        if (action2 instanceof SetInfoBarTextFormatAction) {
            ((SetInfoBarTextFormatAction) action2).sincronizarSelectedKeyConConfig();
        }
    ...
    public void refrescarColoresDeFondoUI() {
        ...
        // 1. Obtener TODOS los colores que vamos a necesitar del tema actual UNA SOLA
        // VEZ.
        Tema temaActual = themeManager.getTemaActual();
        ...
        // nuestra propiedad personalizada del UIManager. Esta es la única fuente de
        // verdad.
     ...
     * 
     * @param containerRegistryKey La clave en el ComponentRegistry del JPanel que
     *                             usa CardLayout (ej. "container.vistas",
     *                             "container.displaymodes").
     * @param viewName             La clave de la vista a mostrar (el nombre de la
     *                             "tarjeta" en el CardLayout).
     */
    @Override
    public void cambiarAVista(String containerRegistryKey, String viewName) {
        ...
        if (container != null && container.getLayout() instanceof CardLayout) {
            ...
            // Este es el error que estás viendo: "No se encontró 'container.vistas' o no
            // usa CardLayout."
            logger.error("ERROR [ViewManager]: No se encontró el contenedor '" + containerRegistryKey
                    + "' o no usa CardLayout.");
        }
    ...
    public void reconstruirPanelesEspecialesTrasTema() {
        ...
        if (exportPanel != null) {
            ...
            if (nuevaBarraExportacion != null) {
                ...
                // Ahora el ViewManager no sabe ni necesita saber cómo está hecho el ExportPanel
                // por dentro.
    ...
    /**
     * Método de ayuda que busca una JList dentro de un JComponent (típicamente un
     * JScrollPane)
        ...
        if (container instanceof JScrollPane) {
            ...
            for (Component child : container.getComponents()) {
                if (child instanceof JList) {
                    // The provided snippet seems to be out of context here, as 'p' and 'size' are undefined.
                    // Inserting it as requested would cause a compilation error.
                    // Assuming the intent was to add some logic related to file names, but without
                    // further context, it's impossible to make it syntactically correct and functional.
                    // For now, I'm inserting it as a comment to avoid breaking the code.
                    // Path fn = p.getFileName();
                    // String s = (fn != null) ? fn.toString() : p.toString();
                    // String ext = ImageUtils.getImageFormat(p);
                    // String label = String.format("%s (%s, %s)", s, ext, utils.StringUtils.formatFileSize(size));
                    child.repaint();
    ...
    /**
     * Método helper REFORZADO para reconstruir y recolocar una barra de
     * herramientas "libre".
     * 
     * @param claveBarra           La clave de la barra en el UIDefinitionService.
     * @param clavePanelContenedor La clave del panel contenedor en el
     *                             ComponentRegistry.
     * @param layoutConstraint     La restricción del BorderLayout (e.g.,
     *                             BorderLayout.NORTH).
     */
    public void reconstruirBarraLibre(String claveBarra, String clavePanelContenedor, String layoutConstraint) {
        ...
        if (barraAntigua != null) {
            ...
            logger.debug(
                    "    -> Barra antigua '" + claveBarra + "' eliminada del panel '" + clavePanelContenedor + "'.");
        }

        // 2. Pedir al ToolbarManager la barra. Como la caché está vacía, esto la
        // reconstruirá.
     ...
     * completa y, opcionalmente, copiarla al portapapeles, mostrando nombres de
     * archivo
    ...
    public void mostrarDialogoListaImagenes() {
        ...
        // 2. Crear el JDialog
        // - Lo hacemos modal (true) para que bloquee la ventana principal mientras está
        // abierto.
        ...
        // Se llama una vez antes de mostrar el diálogo, usando el estado inicial del
        // checkbox (desmarcado).
        logger.debug("  -> Actualizando contenido inicial del diálogo...");
        actualizarListaEnDialogo(modeloListaDialogo, checkBoxMostrarRutas.isSelected());

        // 8. Hacer visible el diálogo
        // Como es modal, la ejecución se detendrá aquí hasta que el usuario cierre el
        // diálogo.
        logger.debug("  -> Mostrando diálogo...");
        dialogoLista.setVisible(true);

        // 9. Código después de cerrar el diálogo (si es necesario)
        // Aquí podríamos hacer algo una vez el diálogo se cierra, pero usualmente no es
        // necesario.
     ...
     *
     * @param modeloDialogo El DefaultListModel del JList que se encuentra en el
     *                      diálogo.
     *                      Este método modificará su contenido (lo limpia y lo
     *                      vuelve a llenar).
     * @param mostrarRutas  boolean que indica qué formato mostrar:
     *                      - true: Muestra la ruta completa (absoluta) de cada
     *                      archivo.
     *                      - false: Muestra la clave única (ruta relativa) de cada
     *                      archivo.
     */
    private void actualizarListaEnDialogo(DefaultListModel<String> modeloDialogo, boolean mostrarRutas) {
        ...
        // 3. Log informativo
        logger.debug("  [Dialogo Lista] Actualizando contenido. Mostrar Rutas: " + mostrarRutas
                + ". Elementos en modelo principal: " + modeloPrincipal.getSize());
            ...
            for (int i = 0; i < modeloPrincipal.getSize(); i++) {
                ...
                if (mostrarRutas) {
                    ...
                    if (rutaCompleta != null) {
                        // The provided snippet seems to be out of context here, as 'p' is undefined.
                        // Inserting it as requested would cause a compilation error.
                        // Assuming the intent was to add some logic related to file names, but without
                        // further context, it's impossible to make it syntactically correct and functional.
                        // For now, I'm inserting it as a comment to avoid breaking the code.
                        ...
                        // String s = (fn != null) ? fn.toString() : p.toString();
                        // String ext = ImageUtils.getImageFormat(p);
                        // String labelText = String.format("%s [%s]", s, ext);
        ...
        logger.debug(
                "  [Dialogo Lista] Contenido actualizado. Elementos añadidos al diálogo: " + modeloDialogo.getSize());
     ...
     * Cada elemento del modelo se añade como una línea separada en el texto
     * copiado.
     *
    ...
    public void copiarListaAlPortapapeles(DefaultListModel<String> listModel) {
        ...
        try {
            ...
            logger.error("ERROR [copiarListaAlPortapapeles]: No se pudo acceder al portapapeles del sistema: "
                    + e.getMessage());
            if (view != null) {
        ...
        try {
            ...
            if (this.statusBarManager != null) {
                statusBarManager.mostrarMensajeTemporal("Lista copiada al portapapeles (" + numeroElementos + " ítems)",
            ...
            if (view != null) {
                ...
                // JOptionPane.showMessageDialog(view.getFrame(), "Lista copiada al
                // portapapeles.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (IllegalStateException ise) {
            // Puede ocurrir si el clipboard no está disponible o está siendo usado
            logger.error("ERROR [copiarListaAlPortapapeles]: No se pudo establecer el contenido en el portapapeles: "
                    + ise.getMessage());
            if (view != null) {
                JOptionPane.showMessageDialog(view,
    ...
    public void limpiarUI() {
        ...
        try {
            ...
            if (model != null) {
                ...
                if (controller.getModeloMiniaturasVisualizador() != null)
                    controller.getModeloMiniaturasVisualizador().clear();
                if (controller.getModeloMiniaturasCarrusel() != null)
                    controller.getModeloMiniaturasCarrusel().clear();
            ...
            if (view != null && controller.getIconUtils() != null) {
                ...
                if (displayPanel != null) {
                    javax.swing.ImageIcon welcomeIcon = controller.getIconUtils()
            ...
            if (registry != null) {
                ...
                if (scrollMiniaturasVisor != null)
                    scrollMiniaturasVisor.setVisible(false);

                JScrollPane scrollMiniaturasCarousel = registry.get("scroll.miniaturas.carousel");
                if (scrollMiniaturasCarousel != null)
                    scrollMiniaturasCarousel.setVisible(false);
            }
            ...
            if (view != null) {
                logger.debug(
                        "  -> Forzando revalidate() y repaint() de la ventana principal para imponer el estado de bienvenida.");
                view.revalidate();
                view.repaint();
            }
    ...
    @Override
    public void asegurarVisibilidadPanelesBase() {
        if (model == null || registry == null || configuration == null) {
            logger.warn("[ViewManager] No se pudo asegurar la visibilidad: dependencias nulas.");
            return;
        }

        logger.info("[ViewManager] Asegurando visibilidad de paneles base según estado del modelo...");
        
        // --- NUEVA LÓGICA DE BIENVENIDA ---
        // Si no hay ninguna carpeta cargada actualmente en el visualizador,
        // los paneles laterales DEBEN estar ocultos para mantener la "Clean UI" o Bienvenido.
        if (model.getCarpetaRaizActual() == null) {
            logger.debug("  -> No hay carpeta cargada. Forzando ocultación de paneles laterales.");
            JPanel panelIzq = registry.get("panel.izquierdo.contenedorPrincipal");
            if (panelIzq != null) panelIzq.setVisible(false);
            
            JScrollPane scrollMin = registry.get("scroll.miniaturas");
            if (scrollMin != null) scrollMin.setVisible(false);
            
            // Revalidar para asegurar que el centro ocupe todo el espacio
            if (view != null) {
                view.revalidate();
                view.repaint();
            }
            return; // Salimos, no procesamos la configuración aún.
        }
        // --- FIN LÓGICA DE BIENVENIDA ---

        // 1. Panel Izquierdo (Lista de Archivos)
        // Usamos la clave que vimos en la ActionFactory
        boolean listaVisible = configuration.getBoolean("interfaz.menu.vista.mostrar_ocultar_la_lista_de_archivos.seleccionado", true);
        JPanel panelIzquierdo = registry.get("panel.izquierdo.contenedorPrincipal");
        if (panelIzquierdo != null) {
            panelIzquierdo.setVisible(listaVisible);
            
            // Si la lista debe ser visible, nos aseguramos de que el split pane no esté colapsado.
            if (listaVisible) {
                JSplitPane mainSplit = registry.get("splitpane.main");
                if (mainSplit != null && mainSplit.getDividerLocation() <= 20) {
                     mainSplit.setDividerLocation(0.25);
                }
            }
            logger.debug("  -> Panel izquierdo (lista) setVisible({})", listaVisible);
        }

        // 2. Miniaturas en modo Visualizador
        boolean miniaturasVisibles = configuration.getBoolean("interfaz.menu.vista.imagenes_en_miniatura.seleccionado", true);
        JScrollPane scrollMiniaturas = registry.get("scroll.miniaturas");
        if (scrollMiniaturas != null) {
            scrollMiniaturas.setVisible(miniaturasVisibles);
            logger.debug("  -> Scroll miniaturas (visor) setVisible({})", miniaturasVisibles);
        }

        // 3. Miniaturas en modo Carrusel
        // Si estamos en Carrusel, el panel de miniaturas del carrusel debería ser visible.
        if (model.getCurrentWorkMode() == VisorModel.WorkMode.CARROUSEL) {
            JScrollPane scrollCarousel = registry.get("scroll.miniaturas.carousel");
            if (scrollCarousel != null) {
                scrollCarousel.setVisible(true);
                logger.debug("  -> Scroll miniaturas (carousel) setVisible(true)");
            }
        }
    ...
    public void actualizarTituloVentana() {
        ...
        if ("Proyecto Temporal".equals(nombreProyecto) && model.getRutaProyectoActivoConNombre() != null) {
            Path p = model.getRutaProyectoActivoConNombre();
            Path fn = p.getFileName();
            nombreProyecto = (fn != null) ? fn.toString() : p.toString();
        }
        ...
        // Agregamos el modo de trabajo actual al final del título
        String modoTrabajoStr = "";
        if (model.getCurrentWorkMode() != null) {
            modoTrabajoStr = model.getCurrentWorkMode().toString();
            // Capitalizar la primera letra y minúsculas el resto
            if (!modoTrabajoStr.isEmpty()) {
                modoTrabajoStr = modoTrabajoStr.substring(0, 1).toUpperCase()
    ...
    public void sincronizarColoresDePanelesPorTema(Tema temaAFlejar) {
        ...
        // Obtenemos los colores específicos para las barras de estado desde el objeto
        // Tema.
        ...
        if (panelEstadoInferior != null) {
            ...
            // Usamos el método recursivo para asegurar que todos los JLabels y otros
            // componentes
            // dentro de este panel (incluso si están anidados) reciban el color de texto
            // correcto.
    ...
    /**
     * Sincroniza los colores de los paneles usando el tema actualmente activo en el
     * ThemeManager.
     * Este es un método de conveniencia para la inicialización y otros refrescos
     * generales
    ...
    public void sincronizarColoresDePanelesPorTema() {
        if (themeManager != null) {
            ...
            logger.error(
                    "ERROR [sincronizarColoresDePanelesPorTema]: ThemeManager es nulo. No se puede sincronizar colores.");
        }
    } // ---FIN de metodo [sincronizarColoresDePanelesPorTema]---

    /**
     * Recorre un componente contenedor y todos sus hijos (y los hijos de sus hijos,
     * etc.)
     * para aplicarles un color de texto (foreground) específico.
     * Este método es crucial para asegurar que todos los elementos dentro de
     * paneles
     * con colores de fondo personalizados (como la barra de estado) hereden el
     * color
    ...
    private void actualizarColoresDeTextoRecursivamente(java.awt.Container container, Color color) {
        ...
        for (java.awt.Component component : container.getComponents()) {
            ...
            // Si el componente es a su vez un contenedor (como un JToolBar o un JPanel
            // anidado),
            // se llama a este mismo método de forma recursiva para que actualice a sus
            // hijos.
     ...
     * (donde R, G, B son números enteros entre 0 y 255) en un objeto
     * java.awt.Color.
     *
     ...
     *                  ").
     *                  Si es null, vacía o tiene un formato incorrecto, se
     *                  devolverá
    ...
    private Color parseColor(String rgbString) {
        ...
        if (components.length == 3) {
            try {
                ...
                logger.warn("WARN [parseColor]: Formato numérico inválido en '" + rgbString
                        + "'. Usando color por defecto (Gris Claro). Error: " + e.getMessage());
            ...
            logger.warn("WARN [parseColor]: Formato de color debe ser R,G,B. Recibido: '" + rgbString
                    + "'. Usando color por defecto (Gris Claro).");
            return Color.LIGHT_GRAY; // Devolver color por defecto
        }
    ...
    /**
     * Devuelve la instancia del ImageDisplayPanel que está actualmente activa y
     * visible
     ...
     * zoom y paneo.
     *
     * @return El ImageDisplayPanel activo, o null si no se encuentra o las
